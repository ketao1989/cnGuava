/*
 * Copyright (C) 2011 The Guava Authors Licensed under the Apache License, Version 2.0 (the "License"); you may not use
...
/**
 * 
 * 为CacheBuilder指定的配置类。CacheBuilderSpec可以支持分析一个字符串的配置，这样可以通过命令行来配置CacheBuilder。
 * 
 * A specification of a {@link CacheBuilder} configuration.
 * 
 * <p>
 * {@code CacheBuilderSpec} supports parsing configuration off of a string, which makes it especially useful for
 * command-line configuration of a {@code CacheBuilder}.
 * 
 * <p>
 ...
 * 
 * <p>
 * Durations are represented by an integer, followed by one of "d", "h", "m", or "s", representing days, hours, minutes,
 * or seconds respectively. (There is currently no syntax to request expiration in milliseconds, microseconds, or
 * nanoseconds.)
 * 
 * <p>
 * Whitespace before and after commas and equal signs is ignored. Keys may not be repeated; it is also illegal to use
 * the following pairs of keys in a single value:
 * <ul>
 ...
 * 
 * <p>
 * {@code CacheBuilderSpec} does not support configuring {@code CacheBuilder} methods with non-value parameters. These
...
public final class CacheBuilderSpec {
    ...
    private final String specification; // 这个字符串就是配置

    private CacheBuilderSpec(String specification) {
        this.specification = specification;
    }// 通过这个字符串来达到配置一个cache的目的
    ...
    public static CacheBuilderSpec disableCaching() {
        ...
        return CacheBuilderSpec.parse("maximumSize=0");// 不缓存，只需要把容器最大值设为0
    }
    ...
    /**
     * Returns a string that can be used to parse an equivalent {@code CacheBuilderSpec}. The order and form of this
     * representation is not guaranteed, except that reparsing its output will produce a {@code CacheBuilderSpec} equal
     * to this instance.
     */
    ...
    /**
     * Returns a string representation for this CacheBuilderSpec instance. The form of this representation is not
     * guaranteed.
     */
    @Override
    ...
    /**
     * Converts an expiration duration/unit pair into a single Long for hashing and equality. Uses nanos to match
